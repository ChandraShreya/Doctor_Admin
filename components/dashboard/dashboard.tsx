...
<div className="rounded-2xl px-6 py-5 
  ...
  <div className="flex items-center justify-between gap-6 flex-wrap">
    ...
    <div>
      ...
      <p className="text-sm md:text-base text-blue-100 dark:text-slate-300 mt-1">
        Manage hospital operations smoothly and efficiently
      </p>
    </div>

    {/* RIGHT STATS BOX (IMPORTANT CHANGE) */}
    {loading ? (
  ...
  <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700">
    ...
    <div className="flex justify-between items-center px-6 py-4 border-b border-slate-100 dark:border-slate-700">
      ...
      <h3 className="font-semibold text-slate-700 dark:text-white text-sm tracking-wide">
        Last 7 Days Confirmed Appointments
      </h3>

      {/* ✅ VIEW ALL BUTTON */}
      <button
        onClick={() => router.push("/appointments?filter=7days")}
    ...
    <div className="px-6 py-4 space-y-3 max-h-[260px] overflow-y-auto">
      ...
      {loading ? (
        ...
        latestConfirmed?.slice(0, 6).map((item) => (
          <div
            key={item._id}
